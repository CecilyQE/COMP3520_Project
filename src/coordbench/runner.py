...
def _retry_sleep_after_failure(attempt_zero_based: int) -> float:
    """Exponential backoff sleep (seconds) before the next attempt.

    Defaults preserve the historical behavior: linear=0, exp_base=3, cap=60
    => min(2**attempt * 3, 60). Override with COORDBENCH_PROVIDER_RETRY_* env.
    """
    linear = _env_float("COORDBENCH_PROVIDER_RETRY_LINEAR_SEC", 0.0)
    exp_base = _env_float("COORDBENCH_PROVIDER_RETRY_EXP_BASE_SEC", 3.0)
    cap = _env_float("COORDBENCH_PROVIDER_RETRY_MAX_SLEEP_SEC", 60.0)
    base = linear + (2**attempt_zero_based) * exp_base
    return float(min(base, cap))


def _success_request_cooldown_seconds() -> float:
    return _env_float("COORDBENCH_SUCCESS_REQUEST_COOLDOWN_SEC", 2.0)
        ...
        def execute(single_request: GenerationRequest):
            ...
            for attempt in range(provider_config.max_retries):
                try:
                    ...
                    cooldown = _success_request_cooldown_seconds()
                    if cooldown > 0:
                        # Slight jitter avoids synchronized bursts when concurrency > 1.
                        time.sleep(cooldown * random.uniform(0.9, 1.1))
                    return {
                        "request": single_request,
                        "response": response,
                        "retry_count": attempt,
                    ...
                    last_error = exc
                    sleep_seconds = _retry_sleep_after_failure(attempt) * random.uniform(0.95, 1.1)
                    LOGGER.warning(
