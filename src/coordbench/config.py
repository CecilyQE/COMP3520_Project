...
def _cfg_int(provider_raw: dict[str, Any], key: str, default: int) -> int:
    """Like int(mapping.get(key)), but treat unset/blank env substitutions as default."""
    v = provider_raw.get(key, default)
    if v is None or v == "":
        return default
    return int(v)


def _cfg_float(provider_raw: dict[str, Any], key: str, default: float) -> float:
    v = provider_raw.get(key, default)
    if v is None or v == "":
...
def load_config(path: str | Path) -> BenchmarkConfig:
    ...
    for provider_name, provider_raw in (raw.get("providers") or {}).items():
        provider_configs[provider_name] = ProviderConfig(
            ...
            max_retries=_cfg_int(provider_raw, "max_retries", 5),
            concurrency=_cfg_int(provider_raw, "concurrency", 3),
            temperature=_cfg_float(provider_raw, "temperature", 1.0),
            max_output_tokens=_cfg_int(provider_raw, "max_output_tokens", 32),
            timeout_seconds=_cfg_int(provider_raw, "timeout_seconds", 120),
            extra={
                k: v
