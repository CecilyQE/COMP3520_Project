...
MAPPED_NORMALIZATION_STATUSES = frozenset({"alias", "human_key", "fuzzy"})
...
def _is_mapped_for_metrics(frame: pd.DataFrame) -> pd.Series:
    if "normalization_status" not in frame.columns:
        return pd.Series(True, index=frame.index)
    status = frame["normalization_status"].fillna("").astype(str)
    return status.isin(MAPPED_NORMALIZATION_STATUSES)


def _exclude_non_metric_rows(frame: pd.DataFrame) -> pd.DataFrame:
    """Remove invalid/unmapped rows so they do not enter distributions or top1 comparisons."""
    if frame.empty or "normalization_status" not in frame.columns:
        return frame
    mapped_mask = _is_mapped_for_metrics(frame)
    excluded = int((~mapped_mask).sum())
    if excluded:
        LOGGER.info("Excluded %s invalid/unmapped rows from metric analysis", excluded)
    return frame[mapped_mask].copy()
...
def _cell_completeness(normalized: pd.DataFrame, config, answer_column: str) -> pd.DataFrame:
    ...
    metric_eligible = _is_mapped_for_metrics(frame) & (frame[answer_column] != "")
    frame["_metric_eligible"] = metric_eligible
    grouped = (
        frame.groupby(CELL_KEYS, dropna=False)
        .agg(
            raw_record_count=(answer_column, "size"),
            successful_samples=("_metric_eligible", "sum"),
        )
        .reset_index()
    )
...
def _ensure_coord_answer(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if "normalization_status" in frame.columns:
        frame.loc[~_is_mapped_for_metrics(frame), "coord_answer"] = ""
    return frame
...
def analyze_run(config_path: str | Path, run_id: str | Path) -> Path:
    ...
    normalized_for_metrics = _exclude_non_metric_rows(normalized_all)
    ...
    cross_lingual_normalized = normalized_for_metrics[normalized_for_metrics["coord_answer_key"] != ""].copy()
    ...
    human_alignment_normalized = normalized_for_metrics[normalized_for_metrics["canonical_answer"] != ""].copy()
