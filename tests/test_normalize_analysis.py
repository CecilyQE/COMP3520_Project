    ...
    config_path.write_text(
        "\n".join(
            [
                ...
                f"  allow_unmapped: {'true' if allow_unmapped else 'false'}",
...
def test_invalid_and_unmapped_excluded_from_cross_lingual_top1(tmp_path: Path, monkeypatch):
    prepared_root = tmp_path / "prepared"
    snapshot = prepared_root / "snapshot"
    _write_prepared_snapshot(snapshot, london_label="old_london", paris_label="old_paris")
    monkeypatch.setattr("coordbench.run_state.prepared_root", lambda: prepared_root)

    clean_config = _write_config(tmp_path, round1_samples=2, allow_unmapped=False)
    clean_dir = tmp_path / "runs" / "metric-eligibility-clean"
    clean_dir.mkdir(parents=True)
    (clean_dir / "run_manifest.json").write_text(
        json.dumps({"run_id": "metric-eligibility-clean", "prepared_snapshot_id": "snapshot", "panel_id": "study2_british_within"}),
        encoding="utf-8",
    )
    clean_rows = [
        _raw_row(prompt_language="en", sample_index=0, response_text="London", request_cache_key="en-0"),
    ...
    with (clean_dir / "raw_generations.jsonl").open("w", encoding="utf-8") as handle:
        for row in clean_rows:
            handle.write(json.dumps(row, ensure_ascii=False) + "\n")
    normalize_run(clean_config, clean_dir)
    analyze_run(clean_config, clean_dir)
    cross = pd.read_csv(clean_dir / "item_metrics.csv")
    cross = cross[cross["metric_family"] == "cross_lingual"]
    assert len(cross) == 1
    assert int(cross.iloc[0]["top1_match"]) == 0

    noisy_config_path = tmp_path / "config-noisy.yaml"
    noisy_config_path.write_text(
        Path(clean_config).read_text(encoding="utf-8").replace("round1_samples: 2", "round1_samples: 12"),
        encoding="utf-8",
    )
    noisy_dir = tmp_path / "runs" / "metric-eligibility-noisy"
    noisy_dir.mkdir(parents=True)
    (noisy_dir / "run_manifest.json").write_text(
        json.dumps({"run_id": "metric-eligibility-noisy", "prepared_snapshot_id": "snapshot", "panel_id": "study2_british_within"}),
        encoding="utf-8",
    )
    noisy_rows = [
        *clean_rows,
        *[
            _raw_row(
                ...
                response_text="President",
                request_cache_key=f"en-president-{index}",
            )
        ...
        _raw_row(
            ...
            sample_index=10,
            response_text="",
            request_cache_key="en-invalid",
            error="503 unavailable",
        ),
    ]
    with (noisy_dir / "raw_generations.jsonl").open("w", encoding="utf-8") as handle:
        for row in noisy_rows:
            handle.write(json.dumps(row, ensure_ascii=False) + "\n")

    with pytest.raises(RuntimeError, match="unresolved outputs"):
        normalize_run(noisy_config_path, noisy_dir)
    analyze_run(noisy_config_path, noisy_dir)

    coord_cell = pd.read_csv(noisy_dir / "coord_cell_completeness.csv")
    en_cell = coord_cell[coord_cell["prompt_language"] == "en"].iloc[0]
    zh_cell = coord_cell[coord_cell["prompt_language"] == "zh"].iloc[0]
    assert int(en_cell["successful_samples"]) == 2
    assert int(zh_cell["successful_samples"]) == 2
